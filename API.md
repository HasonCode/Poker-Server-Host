...
2. **Start-timeout ejection** — `action_timeout_sec` has elapsed since the **first** `/ready` arrived, and at least two players have readied. Any player who did *not* ready is **ejected from the table** (their lobby or seat slot is dropped, their auth token is invalidated, any running bot is killed). The remaining ready cohort is then shuffled into random seats; only those players pay blinds and receive cards. To rejoin, a previously ejected client must `POST /join` again as if they were a new client.
...
- **`start_timeout_sec`** equals the table's `action_timeout_sec` while the gate is active and the cohort is waiting for unanimous ready. **`start_timeout_remaining_sec`** is the live countdown until the start-timeout deadline. It is `null` until the first `/ready` arrives; once it hits zero, anyone still in `waiting_players` is **ejected from the table** (token invalidated, bot killed, lobby/seat slot dropped) and the hand starts with the remaining ready cohort, provided at least two players are ready. If `action_timeout_sec` is `0` (timeouts disabled) the timeout path is disabled entirely and the hand only starts on unanimous ready.
- Leaving the table (`POST /leave` / admin kick) removes the player from `ready_players`. A remaining seat that was previously "all ready" will see the gate re-arm until it reconfirms (or another player joins and signals). If the *last* readied player rescinds (sends `ready: false` or leaves), the start-timeout timer is also cleared and will restart from zero on the next ready.
- Join requests are accepted until the first hand starts. Deferred joins are seated FIFO; newer joins do not bypass older queued joins for the same table. A late joiner does not extend the existing start-timeout — they must ready up before it expires or be ejected.
